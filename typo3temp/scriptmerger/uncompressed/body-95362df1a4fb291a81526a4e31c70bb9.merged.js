...
;jQuery(window).on("load", function () {
	jQuery('.stream-video > .embed-responsive').click(function(evt) {
		var jV = jQuery(this).parent();
		if(!jV.hasClass('loaded')) {
			evt.preventDefault();
			jV.addClass('loaded');
			var src = jV.find('iframe').attr('data-src') + ((jV.find('iframe').attr('data-src').indexOf('?') > 0) ? '&' : '?') + 'autoplay=1';
			jV.find('iframe').attr('src', src).show();
			jV.find('i').fadeOut();
			jV.find('img').fadeOut('slow');
		}			
	});
});
...
var App = function () {
    ...
    function initBootstrapAccordion() {
		// Accordion Open-All (Bootstrap 4)
		if(jQuery('.accordion').length) {
			jQuery('.accordion.open-all .card-header a').attr('aria-expanded','true');
			jQuery('.accordion.open-all .card .collapse').addClass('show');
		};
    }
	...
	function initMaps2(t) {
		window.setTimeout(function() {
			if (typeof google === 'object' && typeof google.maps === 'object') {
				if($.isFunction('initMap'))
					initMap();
			} else if(t < 5) {
				if($.isFunction('initMaps2'))
					initMaps2(t+1);
			}
		},200);
	}
...
function getInnerFullHeight($elements, selector) {
	var h = 0;
	for(var i in $elements)
		h += $elements[i].find(selector).outerHeight();
	return h;
}

/**
 * $pelement: Affix element
 * $aBefore: Elements before to calc offset from
 * keepSpace: boolean wheter to to keep the space of the element
 **/
function updateFix($pelement, $aBefore, keepSpace) {
	$pelementInner = $pelement.find('> div');
	// Set outer height
	$pelement.height($pelementInner.outerHeight());
	// Set fixed top offset
	if(keepSpace) {
		var h = getInnerFullHeight($aBefore, '> div');
		$pelementInner.css('top',h);
	}
}


function osScrollTo(top) {
	// Add little offset space
	// Navbar Toggle always shown, so substract it from scroll position
	var offset = 15,
		navbarToggleHeight = getFixedHeaderHeight();
console.log("osScrollTo", [top, navbarToggleHeight]);
	jQuery('html,body').animate({scrollTop:top-navbarToggleHeight-offset}, '500', 'swing');
}

function getFixedHeaderHeight() {
	return getInnerFullHeight(getFixedHeaderElements(), '> div');
}

function getFixedHeaderElements() {
	var $fhes = [];
	$('.fixed-header-element').each(function() {
		$fhes.push($(this));
	});
	return $fhes;
 ...
 **/
$(function() {
	$('#js-bootstrap-offcanvas').prepend('<i class="fa fa-bars offcanvas-toggle" aria-hidden="true" data-toggle="offcanvas" data-target="#js-bootstrap-offcanvas"></i>');
	$('.offcanvas-toggle').on('click', function(e) {
		e.preventDefault();
		toggleNav();
	});	
	$('#js-bootstrap-offcanvas .nav-link').on('click', function() {
		// If nav-link has href, toggle (close) nav (could be anchor link on same page)
...
function toggleNav() {
	$joc = $("#js-bootstrap-offcanvas");
	$jtoggle = $('.offcanvas-toggle');
	if($joc.hasClass('in')) {
		$joc.removeClass('in');
		$jtoggle.removeClass('is-open');
	} else {
		$joc.addClass('in');
		$jtoggle.addClass('is-open');
	}
}

var NAV_COLLAPSER_OPTS = {
	collapser: '.nav .collapser',
	navLink: '.nav .nav-link',
	subNav: '> ul',
	expandedClass: 'open',
	hiddenClass: 'hidden'
};
 ...
 **/
$(function(opts) {
	$(opts.collapser).click(function(e) {
		e.preventDefault();
		toggle($(this).parent(), opts);
		return false;
	});
	$(opts.navLink).click(function(e) {
		if(typeof $(this).attr('href') === 'undefined') {
			e.preventDefault();
			toggle($(this).parent(), opts);
			return false;
	...
	function toggle($jni, opts) {
		if($jni.hasClass(opts.expandedClass)) {
			$jni.attr('aria-expanded', 'false').removeClass(opts.expandedClass);
			$jni.find(opts.subNav).slideUp();
		} else {
			$jni.attr('aria-expanded', 'true').addClass(opts.expandedClass);
			$jni.find(opts.subNav).hide().removeClass(opts.hiddenClass).slideDown();
		}
	}
}(NAV_COLLAPSER_OPTS));
	...
	if(href.indexOf('#') > 0) {
		...
		if(jQuery(hash).length) {
			window.setTimeout(function() {
				...
				// Accordion collapse scroll to if hidden
				var jaL = jQuery(".accordion .card a[href='"+hash+"']");
				if(jaL.length)
					jaL.click();

				osScrollTo(jQuery(hash).offset().top);
			}, 500);
...
jQuery(document).ready(function() {
	...
	// Panel collapse scroll to if hidden
	jQuery(".collapse").on('shown.bs.collapse', function(){
		var ot = jQuery(this).offset().top,
			wt = jQuery(window).scrollTop();
			
			if(ot < wt)
				osScrollTo(jQuery(this).parent().offset().top);
	});
});

function isUtilityLink($element) {
	var uLinks = ['popup', 'carousel-control', 'carousel-control-prev', 'carousel-control-next'];
	
	// Check all normal utitility links
	for(var i in uLinks)
		if($element.hasClass(uLinks[i]))
			return true;
		
	// If is accordion / panel collapse / data-toggle
	if($element.hasClass('btn-accordion') || typeof $element.attr('data-toggle') != 'undefined')
		return true;
	
	// If is slidedown
	if($element.hasClass('slidedown') || $element.closest('.slidedown').length > 0)
		return true;
 ...
 * Anchor link slider 
 **/
function anchorLinkListener(e) {
	// If link is utility link / has another functionality
	if(isUtilityLink($(this)))
		return;
	
	// If no href anchor present, do nothing
	if(typeof $(this).attr('href') === 'undefined')
		return;
	...
	// If external Url do nothing
	if(href.indexOf('http') >= 0 && href.indexOf(location.host) < 0)
		return;
		
	// If anchor is empty, do nothing
	if(hash === '#')
		return;
	...
	if(jE.length) {
		...
		if(jE.hasClass('panel') && !jE.find('.panel-collapse').hasClass('in')) {
			jE.find('.panel-heading .btn').click();	//jE.collapse('show');
		}
		// Open accordion if necessary
		if(jE.closest('.collapse').length && !jE.closest('.collapse').hasClass('show')) {
			jE = jE.closest('.collapse').parent();
			jE.find('[data-toggle="collapse"]').click();
		}
		
		// Slide to element
		var top = jE.offset().top;
		osScrollTo(top);
	}
}

/** Anchor link slider **/
jQuery('body').on('click', 'a[href*="#"]', anchorLinkListener);


/**
 * Up-Btn link
 **/
jQuery('body').on('click', 'a.up-btn', function(e){
	e.preventDefault();
	osScrollTo(0);
});
...
/**
 * tel-Link action for Desktop
 **/
jQuery('body').on('click', 'a[href^=tel]', function(e){
	if(jQuery(window).width() >= 768) {
		e.preventDefault();
		var m = jQuery('#call-modal');
		if(m.length)
			m.modal('show');
	}
});
...
/**
 * Set popup link listeners
 **/
jQuery('body').on('click', 'a.popup', function(e) {
	e.preventDefault();
	var href = jQuery(this).attr('href'), hash = href.substring(href.indexOf('#'));
	var jm = jQuery(hash);
	if(jm.length)
		jm.modal('show');
});
...
/**
 * DrFlex Popup Listener
 **/
jQuery('body').on('click', '.toggleDrFlexAppointments', function(e){
	e.preventDefault();
	toggleDrFlexAppointments();
});
/**
 * Affix elements to top. 
 * 
 * 25.09.2020
 * @author: Kai Vier
 **/
 
function initFix(options) {
	// Save already fixed elements, to calculate spaces
	var $pesDone = [];
	
	// Collect page-elements as jQuery objs
	var $pes = [];
	for(var i in options.elements) {
		$pes.push($(options.selectorBefore + options.elements[i] + options.selectorAfter));
	}
		
	// Set fixed-classes and do calculations
	for(var i in $pes) {
		$pes[i].addClass(options.className);
		var keep = options.classKeep ? $pes[i].hasClass(options.classKeep) : false;
		// Affix element
		updateFix($pes[i], $pesDone, keep);
		
		// Update sizes/offsets on resize
		$(window).on('resize', () => {
			updateFix($pes[i], $pesDone, keep);
		});
		// Set element to pesDone
		$pesDone.push($pes[i]);
	}
}


var modalTemplate = '<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content">'
+ '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Schließen"><span aria-hidden="true">&times;</span></button><h3 class="modal-title">Modal Title</h3></div>'
+ '<div class="modal-body"></div>'
+ '<div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Schließen</button></div>'
+ '</div></div></div>';

function initPopupContent(jpc) {
	var jM = jQuery(modalTemplate).clone(true,true);
	
	if(jpc.attr('id') == "")
		return;
	
	jM.attr('id', 'popup-'+jpc.attr('id'));
	
	var jTitle = jpc.find('> h1, > h2, > h3, > p');
	jM.find('.modal-title').html(jTitle.html());
	var jBody = jpc.find('.ce-bodytext').html();
	jM.find('.modal-body').html(jBody);
	
	jQuery('body').append(jM);
}
...
function createPopupSimpleModal(id, jThis) {
	$("#"+id).modal();	
}
function createPopupBootstrapModal(id, jThis) {
	var jMd = jQuery('#popup-'+id).length ? jQuery('#popup-'+id) : jQuery('#'+id);
	if(jMd.length) {
		jMd.modal('show');
	}
}
...
const POPUP_TYPE = 'bootstrapmodal';

const AFFIX_OPTIONS = {
	selectorBefore: '#page > ',
	selectorAfter: '.affix',
	classKeep: 'affix-keep-space',
	className: 'fixed-header-element',
	elements: [
		'.page-header', 
		'.page-menu', 
		'.page-banner'
...
/**
 * Overwrite target for internal links (t3 rte bugfix)
 */
jQuery('a.internal-link').attr('target', '_top');
...
jQuery(document).ready(function() {
	// Popups from classes
	jQuery(POPUP_CONTENT_SELECTOR).each(function() {
		var jpc = jQuery(this);
		initPopupContent(jpc);		
	});
	// Auto Popup
	initPopupAuto(POPUP_TYPE);
});

/**
 * Affix page header elements
 */
jQuery(window).on('load', function () {
	initFix(AFFIX_OPTIONS);
});

/**
 * Transition effect for navbar 
 * Options can be set in Typo3 HTML Element like:
	<script>
	var parallaxToggles = [{
		scrollListenerSelector: window, 
		elementSelector: '.page-header > div',
		topOffset: 500,
		className: 'solid'
	}];
	</script>
...
jQuery(document).ready(() => {
	if(typeof parallaxToggles != 'undefined' && parallaxToggles) {
		for(var pt in parallaxToggles) {
			var ptOpts = parallaxToggles[pt];
			initParallaxToggle(
				$(ptOpts.scrollListenerSelector), 
				$(ptOpts.elementSelector), 
				ptOpts.topOffset, 
				ptOpts.className
			);
		}
	}
	//	initParallaxToggle($(window), $('.page-header > div'), 500, 'solid');
});
